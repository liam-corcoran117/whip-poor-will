...
# Data for Common Pauraque 
pauraque_data <- data.frame(
  day = 1:15,
  mass = c(4.40, 7.60, 10.10, 13.40, 17.60, 19.75, 24.60, 28.55, 30.25, 31.25, 36.10, 35.80, 36.90, 38.65, 37.0),
  tarsus = c(9.10, 10.75, 11.75, 13.70, 14.30, 15.00, 15.85, 16.65, 18.45, 19.40, 20.20, 20.25, 20.90, 22.05, 22.00),
  wing = c(0, 0, 0, 21.40, 23.80, 30.60, 35.80, 40.50, 47.30, 57.00, 61.50, 69.50, 79.00, 79.50, 88.00)
)
...
eastern_kingbird_data <- data.frame(
  ...
  wing = c(0, 0, 0, 0.86, 2.6, 5.19, 8.49, 12.43, 16.34, 20.43, 24.36, 28.15, 31.89, 35.13) 
)
...
eastern_phoebe_data <- data.frame(
  ...
  wing = c(0, 0, 0, 0.43, 1.95, 4.24, 7.30, 10.70, 14.27, 18.13, 21.87, 25.41, 28.60, 31.59) 
)
...
# Common Pauraque
fit_mass_pauraque <- nls(mass ~ gompertz(day, a, b, c), data = nightjar_data, start = list(a = 50, b = 1, c = 0.1))
fit_tarsus_pauraque <- nls(tarsus ~ gompertz(day, a, b, c), data = nightjar_data, start = list(a = 14, b = 1, c = 0.1))
fit_wing_pauraque <- nls(wing ~ gompertz(day, a, b, c), data = nightjar_data, start = list(a = 155, b = 1, c = 0.1))
...
params_mass_pauraque <- coef(fit_mass_pauraque)
params_tarsus_pauraque <- coef(fit_tarsus_pauraque)
params_wing_pauraque <- coef(fit_wing_pauraque)
...
time_points_pauraque <- 1:15
growth_rate_summary_mass_pauraque <- calculate_growth_rate_summary(params_mass_pauraque, time_points_pauraque)
growth_rate_summary_tarsus_pauraque <- calculate_growth_rate_summary(params_tarsus_pauraque, time_points_pauraque)
growth_rate_summary_wing_pauraque <- calculate_growth_rate_summary(params_wing_pauraque, time_points_pauraque)
...
growth_rate_summary_mass_pauraque$Variable <- "Pauraque Mass"
growth_rate_summary_tarsus_pauraque$Variable <- "Pauraque Tarsus"
growth_rate_summary_wing_pauraque$Variable <- "Pauraque Wing"
...
combined_growth_rate_summary <- rbind(
  ...
  growth_rate_summary_mass_pauraque,
  growth_rate_summary_tarsus_pauraque,
  growth_rate_summary_wing_pauraque,
...
max_growth_mass_pauraque <- find_max_growth_rate(growth_rate_summary_mass_pauraque)
max_growth_tarsus_pauraque <- find_max_growth_rate(growth_rate_summary_tarsus_pauraque)
max_growth_wing_pauraque <- find_max_growth_rate(growth_rate_summary_wing_pauraque)
...
print(paste("Pauraque Mass: Maximum growth rate is", round(max_growth_mass_pauraque$max_growth_rate, 2), "on day", max_growth_mass_pauraque$max_day))
print(paste("Pauraque Tarsus: Maximum growth rate is", round(max_growth_tarsus_pauraque$max_growth_rate, 2), "on day", max_growth_tarsus_pauraque$max_day))
print(paste("Pauraque Wing: Maximum growth rate is", round(max_growth_wing_pauraque$max_growth_rate, 2), "on day", max_growth_wing_pauraque$max_day))
...
# Function to compute growth rates using linear regression as described in Ramstack et al. 1998 to compare to the swallow species
compute_growth_rate_lm <- function(data, measurement, day_min, day_max) {
  filtered_data <- data %>%
    filter(day >= day_min, day <= day_max) 
  growth_model <- lm(as.formula(paste(measurement, "~ day")), data = filtered_data)
...
# Define time ranges for each variable
day_range_mass <- c(3, 10)   # Mass: Days 3-10
day_range_tarsus <- c(1, 10) # Tarsus: Days 1-10
day_range_wing_nightjar <- c(5, 20)  # Wing for Nightjar: Days 5-20
day_range_wing_others <- c(5, 15)    # Wing for all other species: Days 5-15

# Compute growth rates for Nightjar 
growth_rates_mass_nightjar <- compute_growth_rate_lm(nightjar_data, "mass", day_range_mass[1], day_range_mass[2])
growth_rates_tarsus_nightjar <- compute_growth_rate_lm(nightjar_data, "tarsus", day_range_tarsus[1], day_range_tarsus[2])
growth_rates_wing_nightjar <- compute_growth_rate_lm(nightjar_data, "wing", day_range_wing_nightjar[1], day_range_wing_nightjar[2])

# Compute growth rates for Pauraque 
growth_rates_mass_pauraque <- compute_growth_rate_lm(pauraque_data, "mass", day_range_mass[1], day_range_mass[2])
growth_rates_tarsus_pauraque <- compute_growth_rate_lm(pauraque_data, "tarsus", day_range_tarsus[1], day_range_tarsus[2])
growth_rates_wing_pauraque <- compute_growth_rate_lm(pauraque_data, "wing", day_range_wing_others[1], day_range_wing_others[2])

# Compute growth rates for Kingbird 
growth_rates_mass_kingbird <- compute_growth_rate_lm(eastern_kingbird_data, "mass", day_range_mass[1], day_range_mass[2])
growth_rates_tarsus_kingbird <- compute_growth_rate_lm(eastern_kingbird_data, "tarsus", day_range_tarsus[1], day_range_tarsus[2])
growth_rates_wing_kingbird <- compute_growth_rate_lm(eastern_kingbird_data, "wing", day_range_wing_others[1], day_range_wing_others[2])

# Compute growth rates for Phoebe 
growth_rates_mass_phoebe <- compute_growth_rate_lm(eastern_phoebe_data, "mass", day_range_mass[1], day_range_mass[2])
growth_rates_tarsus_phoebe <- compute_growth_rate_lm(eastern_phoebe_data, "tarsus", day_range_tarsus[1], day_range_tarsus[2])
growth_rates_wing_phoebe <- compute_growth_rate_lm(eastern_phoebe_data, "wing", day_range_wing_others[1], day_range_wing_others[2])

# Combine results into one table with species labels
growth_rates_summary <- bind_rows(
  growth_rates_mass_nightjar %>% mutate(Species = "Nightjar"),
  growth_rates_tarsus_nightjar %>% mutate(Species = "Nightjar"),
  growth_rates_wing_nightjar %>% mutate(Species = "Nightjar"),
  
  growth_rates_mass_pauraque %>% mutate(Species = "Pauraque"),
  growth_rates_tarsus_pauraque %>% mutate(Species = "Pauraque"),
  growth_rates_wing_pauraque %>% mutate(Species = "Pauraque"),
  
  growth_rates_mass_kingbird %>% mutate(Species = "Kingbird"),
  growth_rates_tarsus_kingbird %>% mutate(Species = "Kingbird"),
  growth_rates_wing_kingbird %>% mutate(Species = "Kingbird"),
  
  growth_rates_mass_phoebe %>% mutate(Species = "Phoebe"),
  growth_rates_tarsus_phoebe %>% mutate(Species = "Phoebe"),
  growth_rates_wing_phoebe %>% mutate(Species = "Phoebe")
)
...
# Plot for Common Pauraque
pauraque_plot <- ggplot() +
  geom_line(data = growth_rate_summary_mass_pauraque, aes(x = Day, y = GrowthRate, color = "Pauraque Mass"), size = 1) +
  geom_line(data = growth_rate_summary_tarsus_pauraque, aes(x = Day, y = GrowthRate, color = "Pauraque Tarsus"), size = 1) +
  geom_line(data = growth_rate_summary_wing_pauraque, aes(x = Day, y = GrowthRate, color = "Pauraque Wing"), size = 1) +
  labs(title = "Growth Rate Over Time for Common Pauraque",
       y = "Growth Rate (g/day or mm/day)", x = "Day") +
  theme_minimal() +
  scale_color_manual(values = c("Pauraque Mass" = "black", "Pauraque Tarsus" = "blue", "Pauraque Wing" = "red")) +
  theme(legend.title = element_blank())
